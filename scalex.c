...
int image_speed(const unsigned char* src_ptr, unsigned src_slice, unsigned pixel, unsigned width, unsigned height, int opt_scale_x, int opt_scale_y)
{
	void* dst_alloc = 0;
...
int file_speed(const char* file0, int opt_scale_x, int opt_scale_y)
{
	...
	for (pack = 1; pixel / pack != 0; pack *= 2) {
		...
		image_speed(tmp_ptr, tmp_slice, tmp_pixel, width, height, opt_scale_x, opt_scale_y);
